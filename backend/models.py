...
class Candidate(Base):
    ...
    creator = relationship('User') # Define the relationship
...
class User(Base):
    ...
    company_name = Column(String, nullable=True)  # Only for admin
    num_employees = Column(Integer, nullable=True)  # Only for admin
    logo_url = Column(String, nullable=True) # Add logo_url field for company logo
    ...
    leave_requests = relationship(
        ...
        primaryjoin=lambda:
            User.id == foreign(LeaveRequest.employee_id) and User.company_name == foreign(LeaveRequest.company_name)
    )
    approved_leaves = relationship(
        "LeaveRequest",
        back_populates="approver",
        primaryjoin=lambda:
            User.id == foreign(LeaveRequest.approved_by) and User.company_name == foreign(LeaveRequest.company_name)
    )
    leave_balances = relationship(
        "LeaveBalance",
        back_populates="employee",
        primaryjoin=lambda:
            User.id == foreign(LeaveBalance.employee_id) and User.company_name == foreign(LeaveBalance.company_name)
    )
...
class EmployeeFeedback(Base):
    ...
    # Relationships
    from_employee = relationship("User", back_populates="given_feedbacks", foreign_keys=[from_employee_id])
    to_employee = relationship("User", back_populates="received_feedbacks", foreign_keys=[to_employee_id])
...
class Lead(Base):
    ...
    lead_employer_company_name = Column(String, nullable=True) # Company of the lead
    profession = Column(String, nullable=True)
    qualification = Column(String, nullable=True)
    income = Column(Float, nullable=True)
    date_of_investment = Column(Date, nullable=True)
    investment_type = Column(String, nullable=True)  # equity, sip, lumsum, insurance, PMS, AID, others
    ...
    interaction_type = Column(String, nullable=True)  # call, meeting, etc.
...
class Milestone(Base):
    ...
    description = Column(Text, nullable=True)
    date = Column(Date, nullable=False)
    type = Column(String, nullable=False, default="general")  # Changed from status to type
    created_at = Column(DateTime, server_default=func.now())
...
class Service(Base):
    ...
    interactions = relationship("Interaction", back_populates="service", cascade="all, delete-orphan")
    documents = relationship("ClientServiceDocument", back_populates="service", cascade="all, delete-orphan")
...
class Interaction(Base):
    ...
    managing_company_name = Column(String, nullable=True) # Renamed from company_name
    created_at = Column(DateTime, server_default=func.now())
    service = relationship("Service", back_populates="interactions")

class ClientServiceDocument(Base):
    __tablename__ = "client_service_documents"
    id = Column(Integer, primary_key=True, index=True)
    service_id = Column(Integer, ForeignKey("services.id", ondelete="CASCADE"), nullable=False)
    name = Column(String, nullable=False)
...
class CompanyLeavePolicy(Base):
    ...
    company = relationship("Company", back_populates="leave_policy", foreign_keys=[company_name])
...
class Company(Base):
    ...
    # Define the relationship using primaryjoin with column objects and foreign annotation
    leave_policy = relationship(
        "CompanyLeavePolicy",
        back_populates="company",
        uselist=False  # Ensure it's a one-to-one relationship
    )
